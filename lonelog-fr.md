...
- **Enregistrer ce qui s'est passé** sans ralentir le jeu
- **Suivre les éléments en cours** comme les PNJ, les lieux et les intrigues
- **Partager vos sessions** avec d'autres joueurs solo qui comprendront le format
- **Relire les sessions passées** et retrouver rapidement ce détail crucial d'il y a trois sessions

La notation est conçue pour être :

- **Flexible** — utilisable avec différents systèmes et formats
- **Superposable** — fonctionne à la fois comme un raccourci rapide ou une narration développée
- **Recherchable** — les balises et les codes facilitent le suivi des PNJ, des événements et des lieux
- **Indépendante du format** — fonctionne dans des fichiers markdown numériques ou des carnets analogiques

Les objectifs de la notation :

- **Rendre les rapports écrits par différentes personnes lisibles en un coup d'œil :** les symboles standards facilitent la lecture
- **Séparer la mécanique de la fiction :** les meilleurs rapports sont ceux qui mettent en évidence comment l'utilisation des règles et des oracles informe la fiction
- **Avoir un système modulaire et évolutif :** vous pouvez utiliser les symboles de base ou étendre la notation comme vous le souhaitez
...
À la base, il n'y a que **cinq symboles** (voir *Section 3 : Notation de base*). Ils sont soigneusement choisis pour éviter les conflits avec le formatage markdown et les opérateurs de comparaison. C'est le langage minimal du jeu :
...
C'est tout. **Tout le reste est optionnel.**
...
- **Notation de base** (requise) : Actions, Résolutions, Conséquences
- **Couches optionnelles** (à ajouter au besoin) : Éléments persistants, Suivi de progression, Notes, etc.
- **Structure optionnelle** (pour l'organisation) : En-tête de campagne, En-tête de session, Scènes
...
| Symbole v2.0 | Symbole Lonelog | Pourquoi le changement |
|--------------|-----------------|-------------------------|
...
- **Métadonnées de la campagne** → YAML front matter (en haut du fichier)
- **Titre de la campagne** → Titre de niveau 1
- **Sessions** → Titres de niveau 2 (`## Session 1`)
- **Scènes** → Titres de niveau 3 (`### S1`)
...
En jeu solo, l'incertitude provient de deux sources distinctes : **vous ne savez pas si votre personnage peut faire quelque chose** (c'est la mécanique), ou **vous ne savez pas ce que fait le monde** (c'est l'oracle).
...
**Questions sur le monde / MJ (oracle) :**
...
**Note :** Les opérateurs de comparaison `≥` et `≤` fonctionnent de manière transparente avec la notation lonelog, sans conflit de symboles. Vous pouvez également utiliser `>=` et `<=`.
...
- **Recherche facile** : Trouvez chaque scène où Jonah apparaît
- **Cohérence** : Référencez les PNJ de la même manière à chaque fois
- **Suivi de statut** : Voyez comment les éléments changent au fil du temps
- **Aide-mémoire** : Rappelez-vous des détails des semaines plus tard
...
**Mise à jour des balises de PNJ :**
...
**Quand utiliser les balises de référence :**
...
**Horloges** (se remplissent jusqu'à l'achèvement) :
...
**Utiliser pour :** Menaces qui montent, sorts en préparation, danger qui s'accumule. Quand l'horloge est pleine, quelque chose se passe (généralement mauvais pour vous).

**Pistes** (progression vers un objectif) :
...
**Utiliser pour :** Votre progression sur des projets, l'avancement d'un voyage, des objectifs à long terme. Quand la piste est pleine, vous réussissez quelque chose.

**Compteurs** (comptent à rebours jusqu'à zéro) :
...
**Utiliser pour :** Échéances qui approchent, ressources qui s'épuisent, pression temporelle. Quand il atteint zéro, le temps est écoulé.

**La différence ?** Les horloges et les pistes augmentent toutes deux, mais les horloges sont des menaces (mauvaises quand elles sont pleines) et les pistes sont des progrès (bonnes quand elles sont pleines). Les compteurs diminuent et créent de l'urgence.
...
**Intégration avec les questions à l'oracle :**
...
**Pourquoi enregistrer les lancers ?** Trois raisons :

1.  **Transparence** : Si vous partagez le journal, les autres voient votre processus
2.  **Reproductibilité** : Vous pouvez retracer comment vous avez obtenu des résultats surprenants
3.  **Apprentissage** : Au fil du temps, vous voyez quelles tables vous utilisez le plus
...
**Prose en ligne** (descriptions courtes) :

```
=> La pièce empeste le moisi et la pourriture. Des papiers sont éparpillés partout.
```

**Utiliser pour :** Détails atmosphériques rapides, informations sensorielles, touches émotionnelles. Soyez bref — une phrase ou deux.

**Dialogue** (conversations qui méritent d'être enregistrées) :
...
**Utiliser pour :** Échanges mémorables, voix du personnage, conversations importantes. Vous n'avez pas besoin d'enregistrer chaque mot — juste les échanges qui comptent.

**Longs blocs narratifs** (documents trouvés, descriptions importantes) :
...
**Utiliser pour :** Documents du monde du jeu, longues descriptions, révélations clés. Les marqueurs `\---` et `---` le séparent de votre journal, indiquant clairement qu'il s'agit de contenu fictionnel. Les délimiteurs asymétriques évitent les conflits avec les règles horizontales de Markdown.
...
**Format :** Utilisez des parenthèses pour signaler "ceci est méta, pas de la fiction" :
...
**Quand utiliser les notes méta :**

- **Expériences** : Suivez les variantes de règles ou les règles maison que vous testez
- **Réflexion** : Capturez ce qui a fonctionné ou non émotionnellement
- **Rappels** : Marquez les choses à suivre plus tard
- **Clarification** : Expliquez les décisions inhabituelles ou les interprétations
- **Processus** : Documentez votre pensée pour les journaux partagés

**Quand NE PAS les utiliser :** Ne laissez pas les notes méta submerger votre journal. Si vous vous arrêtez toutes les quelques lignes pour réfléchir, vous sur-analysez probablement. Le jeu est la chose la plus importante — les notes méta ne sont que des commentaires occasionnels en marge.
...
**Les formats numérique et analogique diffèrent ici.** Le markdown numérique utilise le YAML front matter (métadonnées structurées en haut du fichier). Les carnets analogiques utilisent un bloc d'en-tête écrit.

**Pour les fichiers markdown numériques**, utilisez le YAML front matter tout en haut :
...
**Pour les carnets analogiques**, écrivez un bloc d'en-tête de campagne :
...
**Champs optionnels** (à ajouter au besoin) :
...
**Pourquoi utiliser des en-têtes de session ?**

- **Navigation** : Sautez rapidement à des sessions spécifiques
- **Contexte** : Rappelez-vous quand vous avez joué et ce qui se passait
- **Réflexion** : Suivez vos habitudes de jeu (à quelle fréquence ? combien de temps ?)
- **Continuité** : Connectez les sessions avec des résumés et des objectifs
...
**Quand utiliser :** Par défaut pour un jeu linéaire. La scène 2 se passe après la scène 1, la scène 3 après la scène 2, etc.

**Numérotation :** Commencez à S1 à chaque session, ou continuez sur toute la campagne (S1-S100+).
...
**Format :** `T#-S#` où T# est le numéro du fil, S# est le numéro de la scène dans ce fil.
...
**Format :** `S#.#` (par ex., `S5.1`, `S5.2`, `S5.3`)
...
**Utilisez les flashbacks (S5a, S5b) quand :**
...
**Utilisez les fils parallèles (T1-S1, T2-S1) quand :**
...
**Utilisez les montages (S7.1, S7.2) quand :**
...
**À faire : Garder les actions et les jets connectés**
...
**À faire : Utiliser des balises pour les éléments persistants**
...
**À faire : Enregistrer clairement les conséquences**
...
**À faire : Utiliser des balises de référence dans les scènes ultérieures**
...
**À faire : Mélanger raccourci et narration au besoin**
...
**À ne pas faire : Enfouir la mécanique dans la prose**
...
**À ne pas faire : Oublier d'enregistrer les conséquences**
...
**À ne pas faire : Perdre le fil des balises entre les scènes**
...
**À ne pas faire : Mélanger les symboles d'action et d'oracle**
...
**À ne pas faire : Oublier le contexte de la scène**
...
Chaque modèle est disponible en formats **markdown numérique** et **carnet analogique**. Choisissez celui qui correspond à votre style de jeu, ou utilisez-les comme inspiration pour créer vos propres modèles.
...
**Q : Dois-je utiliser tous les éléments ?**
R : Non ! Commencez avec juste `@`, `?`, `d:`, `->`, et `=>`. N'ajoutez d'autres éléments que s'ils vous aident.

**Q : Puis-je utiliser ceci avec des JDR traditionnels (avec un MJ) ?**
R : La notation de base fonctionne très bien pour toutes les notes de JDR. Les éléments de l'oracle (`?`, `->`) sont spécifiquement pour le jeu solo, mais la notation action/résolution fonctionne partout.

**Q : Et si mon système n'utilise pas de dés ?**
R : Utilisez `d:` pour toute mécanique de résolution : `d: Tirer une carte -> Dame de Pique`, `d: Dépenser un jeton -> Succès`

**Q : Dois-je utiliser le format numérique ou analogique ?**
R : Celui que vous préférez ! Ils utilisent la même notation. Le numérique a une meilleure recherche/organisation ; l'analogique est immédiat et tactile.

**Q : À quel point mes notes doivent-elles être détaillées ?**
R : Aussi détaillées que vous le souhaitez ! Le système fonctionne pour du raccourci pur (Exemple 6.1) ou une narration riche (Exemple 6.4).

**Q : Puis-je partager mes journaux avec d'autres ?**
R : Oui ! C'est l'une des raisons d'une notation standardisée. D'autres personnes familières avec le système peuvent lire vos journaux facilement.

**Q : Qu'en est-il des règles maison ou des symboles personnalisés ?**
R : Documentez-les dans des notes méta : `(note : utilisation de + pour l'avantage, - pour le désavantage)`. Le système est conçu pour être étendu.

**Q : Les numéros de scène doivent-ils être séquentiels ?**
R : Non. Utilisez `S1`, `S2`, `S3` pour la simplicité, mais créez des branches (`S3a`, `S3b`) ou utilisez des préfixes de fil (`T1-S1`) si cela est utile.
...
Cette notation est inspirée de la [Valley Standard](https://alfredvalley.itch.io/the-valley-standard).
